...
export default function HUD({ game, arenaW, arenaH, onExit, muted, onToggleMute }) {
  ...
  return (
    <>
      ...
      {/* TOP BAR */}
      <div className="absolute top-3 left-0 right-0 z-10 flex items-start justify-between px-6 py-4 pointer-events-none">
        <div className="pointer-events-auto">
          <div className="font-marker text-2xl ink-text-gold tracking-wider" data-testid="hud-title">
            KRONOS <span className="font-splat ink-text-purple text-xl">INK PARK</span>
          </div>
          <div className="text-xs font-terminal text-[#6b5d7a] mt-1">
        ...
        <div className="flex items-center gap-6 pointer-events-auto">
          <div className="ink-panel px-5 py-3 text-center min-w-[120px]" data-testid="hud-timer">
            <div className="text-[10px] font-terminal text-[#6b5d7a] tracking-widest">TIMER</div>
            <div
              className={`text-3xl font-marker ${remaining < 15 ? "ink-text-blood ink-pulse" : "ink-text-toxic"}`}
          ...
          <div className="ink-panel px-4 py-3 flex gap-5">
            <div className="text-center" data-testid="hud-survivors">
              <div className="flex items-center gap-1 text-[10px] font-terminal text-[#6b5d7a] tracking-widest">
                <Shield size={10} className="text-[#fbbf24]" /> SKATERS
              </div>
              <div className="text-2xl font-marker ink-text-gold">{survivors.length}</div>
            </div>
            <div className="text-center" data-testid="hud-infected">
              <div className="flex items-center gap-1 text-[10px] font-terminal text-[#6b5d7a] tracking-widest">
                <Skull size={10} className="text-[#a855f7]" /> INFECTED
              </div>
              <div className="text-2xl font-marker ink-text-purple">{infected.length}</div>
            </div>
          </div>
        </div>
      ...
      <div className="absolute top-24 left-1/2 -translate-x-1/2 z-10 pointer-events-none">
        <div
          className="ink-panel px-5 py-2 text-sm font-marker tracking-widest"
  ...
  return (
    <div className="absolute bottom-6 right-6 z-10 pointer-events-none" data-testid="hud-minimap">
      <div className="ink-panel p-2">
        <div className="text-[10px] font-terminal text-[#6b5d7a] tracking-widest mb-1 px-1">[ MAP ]</div>
        <div
